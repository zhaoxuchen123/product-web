...
const boards = [
  {
    ...
    diffs: [
      ...
      { label: 'PCI ID',  value: '0x8848:0x8308',  code: true  },
...
<div class="plat-hero">
  <div class="plat-hero-left">
    <div class="plat-breadcrumb"><a href="/products/nic-driver/">网卡驱动库</a> / 沐创系列 / rnp500</div>
    <h1 class="plat-title">rnp500</h1>
    <p class="plat-mfr">沐创（MUCSE）· PCIe 万兆 N500</p>
...
rnp500 驱动以内核模块（.ko）方式发布，运行时动态加载：
...
# 卸载
rmmod libdrv_rnp_n500
```

加载后驱动自动枚举 PCIe 总线上的沐创 N500 设备，按顺序创建 `eth0`、`eth1`...
...
rnp500 驱动支持以下沐创万兆以太网控制器：

| 型号 | PCI Device ID | 接口 | 备注 |
|------|---------------|------|------|
| N500 | 0x8308 | SFP+ | 万兆型号 |
